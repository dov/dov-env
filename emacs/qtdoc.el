...
;; Changed and simplified by Dov Grobgeld
...
;; Changed by Dov to always use browse-url and not w3m
...
(defun qtdoc-lookup ()
  ...
  (let* ((info-lookup-mode 'c-mode)
         ...
         (name
          (completing-read
           (if default
               (format "QtClass (default %s): " default)
             "QtClass: ")
             qtdoc-classes nil nil nil
             'info-lookup-history default)))
    (browse-url (concat qtdoc-html-root "/" (downcase name) ".html"))))
