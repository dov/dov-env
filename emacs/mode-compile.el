...
;; Override in order not to get prompted about perl arguments.
(defun mc--read-string (prompt &optional initial-contents)
  "")
...
(defun mc--shell-compile (shell dbgflags &optional errors-regexp-alist)
  ...
  (let* ((shcmd   (or (mc--which shell)
         ...
         (run-cmd (concat shcmd " " dbgflags " " shfile " "
                          (setq mc--shell-args
                                (mc--read-string (if mode-compile-expert-p
                                                     "Argv: "
