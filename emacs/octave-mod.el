		...
		(while (< (point) eol)
		  (if (octave-not-in-string-or-comment-p)
			...
			(if (and (not (= bot (point)))
                                 (not (octave-end-as-array-index-p))) ;special case for end keyword but is applied to all keywords
			    (setq icol (- icol
...
;; TODO: this should probably also make sure we are actually looking at and "end" keyword
