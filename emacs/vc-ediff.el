...
;;
;; Dov Grobgeld: Changed vc-version-other-window to vc-revision-other-window
...
(defun vc-ediff (arg)
  ...
  (let ((work (current-buffer))
    ...
    (vc-revision-other-window rev) ;; let vc do all the work
...
This would normally be put in your `init.el' file."
  ;; SAM Broken. Since I no longer use CVS for anything, I'm not going to
  ;; bother fixing it.
  (when nil
    (require 'vc)
